...
Each screening row adds `route_diagnostic` with state, reason_codes, next_action, method_version and biological_impossibility_claim=false. TSV adds route_state, next_action, primary_annotation_support, primary_candidate_comparison, receiver_review_status. `candidate_comparison.tsv` has one row per candidate including blocked rows; `receiver_review_plan.tsv` has one row per candidate/check. No new full-fusion export is introduced.

`manual_review_queue.tsv` is a purposive, deterministic subset, not a benchmark with known truth. Its stratum includes scope/topology/class/route/boundary-support/integrity/epitope status. `review_status=pending`, independent_boundary/reviewer/source/disagreement_reason and four endpoint fields are blank. Blank is missing, not failure. It is not accepted as assembly approval. Per-stratum limit (1-100) is included in run options and identity hash. Full contracts and limitations: [methodology](methodology-v050.md).
...
- `reference_selection`: basis (`database_canonical`, `explicit_isoform`, or `unresolved_noncanonical`), selected reference, rationale, `experimental_isoform_confirmation`, `selected_sequence_sha256` and `selected_entry_version`. A canonical entry uses its uniquely annotated Displayed isoform ID; without one, `accession:canonical` is an analysis marker, not an invented biological isoform. A noncanonical request cannot relabel canonical-entry coordinates. Canonical selection is not laboratory confirmation; unconfirmed selection blocks fusion assembly only. The inventory count and `isoform_comparison=not_evaluated` remain explicit.
...
Since v0.4.2 `canonical_isoform_ids`, `reference_coordinate_status`, `raw_locations`, `reference_locations`, `has_secreted_reference_annotation` and `excluded_annotations` preserve annotation applicability. A foreign `feature.location.sequence` is excluded from reference coordinates, not silently remapped. Molecule-scoped comments apply only when matched to the selected displayed isoform or an exact principal-name mature chain ending at the reference terminus; other/uncertain contexts are preserved for review. Generic `Membrane` becomes `membrane_unspecified`; named cell-membrane sublocations are distinguished from organelles. Mixed surface/secreted annotation without TM can support a secreted-extension route but does not prove an integral natural surface target. See [audit policy](audit-v042.md).
...
Candidate JSON/TSV independently records `screening_recommendation`, `screening_reason_codes`, `screening_rationale`, plus the separate `protein_screening_recommendation`. A blocked alternate cannot inherit a positive primary recommendation. Excluded annotation contexts are included in the reference screening evidence; they are not negative findings.
